...
trait BaseModel
{
    ...
    private function genBaseModel($where, string $pkName, string $isOr = "and")
    {
        ...
        if (strtolower($isOr) === "or") {
            $model = is_array($where) || $where instanceof \Closure ? static::where(function ($query) use ($where) {
                $query->whereOr($where);
     ...
     * @param bool $isAll 是否返回全部数据
     * @param bool $isModel 是否返回模型对象
     * @return $this|array|bool 默认返回编辑以后的数据全部内容
     */
    public function edit(array $data, bool $isAll = true, bool $isModel = false)
    {
        try {
            // 验证主键是否存在
            $idName = $this->getPk();
            ...
            if ($res && $isAll) {
                $res = self::where($idName, $data[$idName])->find();
            }
            if ($isModel) {
                return $res;
            }
    ...
    public function selectChild(
        ...
        try {
            ...
            if (strtolower($isOr) === "or") {
                $model = is_array($where) || $where instanceof \Closure ?
                    static::alias($alias)->where(function ($query) use ($where) {
                        $query->whereOr($where);
                    }) : static::alias($alias)->where("this." . $idName, $where);
            } else {
                $model = is_array($where) || $where instanceof \Closure ?
                    static::alias($alias)->where($where) : static::alias($alias)->where("this." . $idName, $where);
            }
